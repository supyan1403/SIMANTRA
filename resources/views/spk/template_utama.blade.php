...
<head>
    ...
    <style>
        ...
        .page { width: 210mm; min-height: 297mm; padding: 20mm 20mm 20mm 25mm; margin: 10px auto; background: white; box-shadow: 0 0 10px rgba(0,0,0,0.1); border-radius: 4px; }
        ...
        table.table-bordered th, table.table-bordered td { border-color: #000 !important; padding: 6px 8px; }
...
<div class="no-print text-center py-3 bg-dark text-white sticky-top">
    <button onclick="window.print()" class="btn btn-danger font-monospace px-4"><i class="bi bi-printer me-2"></i>CETAK FILE UTAMA SPK (PDF / PRINTER)</button>
...
<div class="page">
    ...
    <div class="spk-nomor">Nomor: B-{{ sprintf('%04d', $mitra->id) }}/BPS/3206/SPK/{{ date('m/Y') }}</div>
    ...
    <table class="ms-3 mb-3">
        ...
        <tr>
            ...
            <td>: {{ $mitra->pekerjaan ?? 'Mitra Statistik BPS' }}</td>
        </tr>
        <tr>
            <td></td>
            <td><strong>Alamat</strong></td>
            <td>: {{ $mitra->alamat ?? 'Kabupaten Tasikmalaya' }}</td>
        </tr>
        <tr>
            <td></td>
    ...
    <p>PIHAK PERTAMA dan PIHAK KEDUA secara bersama-sama sepakat untuk mengikatkan diri dalam Surat Perjanjian Kerja (SPK) Pelaksanaan Kegiatan Statistik dengan ketentuan sebagai berikut:</p>
    ...
    <ol>
        ...
        <li>PIHAK KEDUA berhak menerima pembayaran honorarium sebesar <strong>Rp {{ number_format($totalHonor, 0, ',', '.') }}</strong> ({{ ucwords(trim(preg_replace('/(?<=\d)(?=(\d{3})+$)/', '.', (string)$totalHonor))) }} Rupiah) setelah pekerjaan diselesaikan dan diterima dengan baik oleh PIHAK PERTAMA.</li>
    </ol>
