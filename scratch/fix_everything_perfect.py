...
# 1. LOAD ORIGINAL PRISTINE KENDI ON BLACK BG
...
# Compute true brightness for alpha
max_c = np.maximum(np.maximum(r, g), b)

# Create mask: pure black background has max_c < 12
# Inside the kendi, even dark shadows have max_c > 15
alpha = np.zeros_like(max_c)
alpha[max_c >= 25] = 255.0
soft = (max_c >= 8) & (max_c < 25)
alpha[soft] = ((max_c[soft] - 8.0) / 17.0) * 255.0

# Inside the Kendi bounding area, ensure full opacity (NO HOLES, NO DIVISION NOISE)
yy, xx = np.ogrid[:arr.shape[0], :arr.shape[1]]
# Inside the main body (y: 100..900, x: 180..820):
kendi_core = (yy > 80) & (yy < 890) & (xx > 180) & (xx < 820) & (max_c > 10)
alpha[kendi_core] = 255.0

# DO NOT DIVIDE RGB BY ALPHA! Keep original rich 3D colors exactly as rendered:
out_r = np.clip(r, 0, 255)
out_g = np.clip(g, 0, 255)
out_b = np.clip(b, 0, 255)

rgba = np.dstack((out_r, out_g, out_b, alpha)).astype(np.uint8)
...
# Scale to 900x900 inside 1000x1000 canvas
...
print("\n--- ALL 6 FILES PERFECTLY SYNCED WITH PREMIUM METALLIC 3D TEXT & ZERO LEAK ---")
